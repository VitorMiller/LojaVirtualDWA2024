...
async def excluir_produtos(inputDto:IdProdutoDTO) :
    ...
    pb = ProblemDetailsDto("int",f"O produto com id <b>{inputDto.id_produto}</b> não foi encontrado.", "value_not_found", ["body", "id_produto"])
...
async def alterar_produtos(inputDto:AlterarProdutoDTO) :
    ...
    pb = ProblemDetailsDto("int",f"O produto com id <b>{inputDto.id}</b> não foi encontrado.", "value_not_found", ["body", "id_produto"])
    return JSONResponse(pb.to_dict(),status_code=404) 